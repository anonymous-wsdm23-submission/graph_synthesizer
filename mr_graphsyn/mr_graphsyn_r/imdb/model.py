...
from norm import normalize_adjacency
...
class GCNNet(nn.Module):
    ...
    def __init__(self, n_layer, in_dim, hidden_dim, out_dim, num_head, pred_dim, class_cardinality, dropout_rate, attention_dropout_rate, indices_neighbor, num_edge):
        ...
        self.block = GCNBlock(n_layer, in_dim, hidden_dim, class_cardinality, self.activation, num_head, dropout_rate, attention_dropout_rate, indices_neighbor, num_edge)
    ...
    def forward(self, x, adj, target_X, target):
        ...
        out, adj, beta_list = self.block(x, adj)
        ...
        return loss, y[target_X], y, beta_list
...
class GCNBlock(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        for i, layer in enumerate(self.layers):
            out, adj, tmp = layer((x if i == 0 else out), adj)
            beta_list.append(tmp)

        return out, adj, beta_list
...
class GCNLayer(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        out, beta_list = self.attention(out, adj)
...
class Attention(nn.Module):
    ...
    def __init__(self, out_dim, num_head, indices_neighbor, num_edge, dropout_rate, attention_dropout_rate):
        ...
        self.att_layers = list()

        for i in range(self.num_head):
            self.att_layers.append(list())

        for i in range(self.num_head):
            for j in range(self.num_edge):
                self.att_layers[i].append(torch.Tensor(12772, 12772))
    ...
    def forward(self, x, adj):
        ...
        for i in range(self.num_head):
            ...
            for j, indices in enumerate(self.indices_neighbor):
                ...
                att_score = self.leaky_relu(self.att_layers[i][j])
            ...
            beta = F.softmax(self.att_weights[i], dim=-1)
            beta_list.append(beta)

            # 1x1 conv
            alpha = torch.sum(torch.mul(attended_edge, beta), dim=-1)
