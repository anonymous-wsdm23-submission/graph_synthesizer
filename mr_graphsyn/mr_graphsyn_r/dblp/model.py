...
class Attention(nn.Module):
    ...
    def __init__(self, out_dim, num_head, indices_neighbor, num_edge, dropout_rate, attention_dropout_rate):
        ...
        for i in range(self.num_head):
            for _ in range(self.num_edge):
                self.att_layers[i].append(torch.Tensor(18405, 18405))
        ...
        self.zero_vec = -9e15*torch.ones(18405,18405)
    ...
    def forward(self, x, adj):
        ...
        for i in range(self.num_head):
            ...
            for j, adj_type in enumerate(adj):
                att_score = self.leaky_relu(self.att_layers[i][j])
                att_score = torch.where(adj_type > 0, att_score, self.zero_vec)
                att_ratio = F.softmax(att_score, dim=-1)
            ...
            alpha = torch.where(alpha > 0, alpha, self.zero_vec)
