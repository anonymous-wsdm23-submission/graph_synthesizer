...
class Attention(nn.Module):
    ...
    def __init__(self, out_dim, num_head, indices_neighbor, dropout_rate, attention_dropout_rate):
        ...
        self.att_pattern = nn.ModuleList()
        for _ in range(self.num_head):
            self.att_pattern.append(nn.Linear(self.out_dim, 18405))

        for att in self.att_pattern:
            nn.init.xavier_uniform_(att.weight)
            att.weight.required_grad = False
            att.bias.required_grad = False
    ...
    def forward(self, x, adj, is_val, epoch, layer_position):
        ...
        for i in range(self.num_head):
            ...
            att_score = self.leaky_relu(self.att_pattern[i](x_transformed))
