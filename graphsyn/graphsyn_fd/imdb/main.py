    ...
    for split in split_list:
        ...
        for cnt in range(count):
            ...
            for i in range(epochs):
                ...
                # wegiht(filter matrix)        
                    # 0st layer
                    layer0_weight = []
                    # 0st layer - 0st head
                    layer0_weight.append(model.block.layers._modules['0'].filter_matrix_list._modules['0'].weight.data.clone().detach().requires_grad_(False).cpu())
                    # 0st layer - 1st head
                    layer0_weight.append(model.block.layers._modules['0'].filter_matrix_list._modules['1'].weight.data.clone().detach().requires_grad_(False).cpu())
                    # 0st layer - 2nd head
                    layer0_weight.append(model.block.layers._modules['0'].filter_matrix_list._modules['2'].weight.data.clone().detach().requires_grad_(False).cpu())

                    # 1st layer
                    layer1_weight = []
                    # 1st layer - 0st head
                    layer1_weight.append(model.block.layers._modules['1'].filter_matrix_list._modules['0'].weight.data.clone().detach().requires_grad_(False).cpu())
                    # 1st layer - 1st head
                    layer1_weight.append(model.block.layers._modules['1'].filter_matrix_list._modules['1'].weight.data.clone().detach().requires_grad_(False).cpu())
                    # 1st layer - 2nd head
                    layer1_weight.append(model.block.layers._modules['1'].filter_matrix_list._modules['2'].weight.data.clone().detach().requires_grad_(False).cpu())

                # bias(filter matrix)        
                    # 0st layer
                    layer0_bias = []
                    # 0st layer - 0st head
                    layer0_bias.append(model.block.layers._modules['0'].filter_matrix_list._modules['0'].bias.data.clone().detach().requires_grad_(False).cpu())
                    # 0st layer - 1st head
                    layer0_bias.append(model.block.layers._modules['0'].filter_matrix_list._modules['1'].bias.data.clone().detach().requires_grad_(False).cpu())
                    # 0st layer - 2nd head
                    layer0_bias.append(model.block.layers._modules['0'].filter_matrix_list._modules['2'].bias.data.clone().detach().requires_grad_(False).cpu())

                    # 1st layer
                    layer1_bias = []
                    # 1st layer - 0st head
                    layer1_bias.append(model.block.layers._modules['1'].filter_matrix_list._modules['0'].bias.data.clone().detach().requires_grad_(False).cpu())
                    # 1st layer - 1st head
                    layer1_bias.append(model.block.layers._modules['1'].filter_matrix_list._modules['1'].bias.data.clone().detach().requires_grad_(False).cpu())
                    # 1st layer - 2nd head
                    layer1_bias.append(model.block.layers._modules['1'].filter_matrix_list._modules['2'].bias.data.clone().detach().requires_grad_(False).cpu())
