...
if __name__ == '__main__':
    ...
    for split in split_list:
        ...
        node_label_3 = node_num_label_per_class[3]
        ...
        random.seed(0)
        random.shuffle(node_label_3)

        train_node = []
        train_target = []

        splitted = int(split * 200)
        for (n0, n1, n2, n3) in zip(node_label_0[:splitted], node_label_1[:splitted], node_label_2[:splitted], node_label_3[:splitted]):
            train_node.append(n0[0])
            train_node.append(n1[0])
            train_node.append(n2[0])
            train_node.append(n3[0])

            train_target.append(n0[1])
            train_target.append(n1[1])
            train_target.append(n2[1])
            train_target.append(n3[1])
        ...
        for cnt in range(count):
            ...
            with open(str(cnt) + '.best_out.pickle', 'wb') as f:
